...
file: Vector2fArrrayConverter.cs
...
{
    public class WinCoreVector2fArrayTypeConverter : CoreVector2fArrayTypeConverter
    {
        ...
        public override object ConvertFrom(ITypeDescriptorContext context, System.Globalization.CultureInfo culture, object value)
        {
            if (value is string)
            {
                ...
                Vector2f[] v_r = new Vector2f[v_t.Length];
                for (int i = 0; i < v_r.Length; i++)
                {
                    v_r[i] = Vector2f.ConvertFromString(v_t[i]);
                }
                return v_r;
            }
            if (value is Point[])
            {
                Point[] pts = value as Point[];
                Vector2f[] v_t = new Vector2f[pts.Length];
                for (int i = 0; i < v_t.Length; i++)
                {
                    v_t[i] = new Vector2f(pts[i].X, pts[i].Y);
                }
                return v_t;
            }
            if (value is PointF [])
            {
                PointF[] pts = value as PointF[];
                Vector2f[] v_t = new Vector2f[pts.Length];
                for (int i = 0; i < v_t.Length; i++)
                {
                    v_t[i] = new Vector2f(pts[i].X, pts[i].Y);
