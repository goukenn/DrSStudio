...
namespace IGK.DrSStudio.Imaging
{
    ...
    public static class BitmapExtensions
    {
            ...
            bool forceLegacy)
        {
            ...
            GdiNativeFunction.CheckEnumRange<HistogramFormat>(format, HistogramFormat.ARGB, HistogramFormat.A, "format");

            int liStatus;

            uint luiNumberOfEntries;

            if (!forceLegacy && GdiInfo.Ver11Available)
            {
                liStatus = GdiNativeFunction.GdipBitmapGetHistogramSize((GdiNativeFunction.GpHistogramFormat)format, out luiNumberOfEntries);
                ...
                switch (format)
                {
                    ...
                    case HistogramFormat.Gray:
                        BitmapPerPixelProcessing.ApplyPerPixel(
                            ...
                            { 
#if Unsafe
                                channel0[GdiNativeFunction.LuminosityF(
                                    ((byte*)d)[2], 
                                    ((byte*)d)[1], 
                                    ((byte*)d)[0])
                                ]++;
        ...
        private static void GetHistogramGDIPlus(Bitmap bitmap, HistogramFormat format, uint[] channel0, uint[] channel1, uint[] channel2, uint[] channel3, uint numberOfEntries)
        {
            ...
            unsafe
            {
                ...
                {
                    liStatus = GdiNativeFunction.GdipBitmapGetHistogram(
                        new HandleRef(bitmap, bitmap.NativeHandle()),
                        (GdiNativeFunction.GpHistogramFormat)format, numberOfEntries,
                        (IntPtr)luiChannel0, (IntPtr)luiChannel1, (IntPtr)luiChannel2, (IntPtr)luiChannel3);
                }
            }

#else
                GCHandle[] lhHandles = GdiNativeFunction.PinObjects(channel0, channel1, channel2, channel3);
                ...
                {
                    GdiNativeFunction.UnpinObjects(lhHandles);
                }
#endif
