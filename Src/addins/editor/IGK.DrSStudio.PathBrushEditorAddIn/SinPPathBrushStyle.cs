...
{
    public class SinPPathBrushStyle : SinPathBrushStyle
    {
        static SinPPathBrushStyle()
        {
            IGK.DrSStudio.Drawing2D.CorePathBrushStyleBase.Register(PathBrushConstant.SIN_STYLE+"2",
        ...
        public override void Generate(System.Drawing.Drawing2D.GraphicsPath path)
        {
            ...
            for (int i = 0; i < c.Length; i++)
            {
                ...
                //draw 
                mp.AddLine(c[i].X,
                    (float)(c[i].Y - Math.Abs(this.Amplitude * Math.Cos(i * this.Period * Math.PI / (c.Length - 1)))),
                    c[i].X,
                    (float)(c[i].Y + Math.Abs (this.Amplitude * Math.Cos(i * this.Period * Math.PI / (c.Length - 1)))));
                mp.Transform(mat);
